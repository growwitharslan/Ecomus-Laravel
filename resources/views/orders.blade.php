                        ...
                        <table class="table table-striped table-hover">
                            ...
                            <tbody>
                                @foreach($orders as $order)
                                    <tr>
                                        <td data-label="#" class="d-block d-md-table-cell">
                                            {{ $loop->iteration }}
                                        </td>
                                        <td data-label="Amount" class="d-block d-md-table-cell">
                                        ...
                                        <td data-label="Status" class="d-block d-md-table-cell">
                                            ...
                                            @endphp
                                            <span class="badge {{ $badgeClasses[$order->status] ?? 'bg-secondary' }}">
                                                {{ ucfirst($order->status) }}
                                            </span>
                                        </td>
                                        <td data-label="Actions" class="d-block d-md-table-cell">
                                            <a href="{{ route('order.details', $order->id) }}" 
                                               class="btn btn-sm btn-primary mb-1 mb-md-0">
                                               View
                                            </a>
                                            <button type="button" 
                                                    class="btn btn-sm btn-danger btn_delete" 
                                                    data-id="{{ $order->id }}" 
                                                    data-text="Are you sure you want to undo this order?">
                                                Cancel
                                            </button>
                                        </td>
                                    </tr>
                    ...
                    @else
                        <div class="text-center">
                            <img class="img-fluid" style="max-width: 300px;" 
                                 src="{{ asset('assets/thenounproject.png') }}" 
                                 alt="Nothing in Orders">
